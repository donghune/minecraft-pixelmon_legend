package io.github.hoya.pixelmonlegend.util.serialization.serializers

import io.github.hoya.pixelmonlegend.util.serialization.serializers.exceptions.WorldNotFoundException
...
object LocationSerializer : KSerializer<Location> {
    ...
    fun deserializeIndependent(raw: String): Location {
        val slices = raw.split(LOCATION_SEPARATOR)
        val worldName = slices[0]
        val world = Bukkit.getWorld(worldName) ?: throw WorldNotFoundException(worldName)
    ...
    fun serializeIndependent(value: Location): String {
        return "${value.world.name}$LOCATION_SEPARATOR${value.x}$LOCATION_SEPARATOR${value.y}$LOCATION_SEPARATOR${value.z}$LOCATION_SEPARATOR${value.yaw}$LOCATION_SEPARATOR${value.pitch}"
    }
}